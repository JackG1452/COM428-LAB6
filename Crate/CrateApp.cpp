...
void CrateApp::UpdateObjectCBs(const GameTimer& gt)
{
	...
	for(auto& e : mAllRitems)
	{
		...
		//	if (e->ObjCBIndex = 0) {
				XMMATRIX rot = XMMatrixRotationY(XMConvertToRadians(rotateY));
				world = world * rot;
...
void CrateApp::LoadTextures()
{
	...
	auto brickCrateTex = std::make_unique<Texture>();
	brickCrateTex->Name = "brickCrateTex";
	brickCrateTex->Filename = L"Textures/bricks.dds";
	ThrowIfFailed(DirectX::CreateDDSTextureFromFile12(md3dDevice.Get(),
		mCommandList.Get(), brickCrateTex->Filename.c_str(),
		brickCrateTex->Resource, brickCrateTex->UploadHeap));

	mTextures[brickCrateTex->Name] = std::move(brickCrateTex);
}
...
void CrateApp::BuildDescriptorHeaps()
{
	...
	auto brickCrateTex = mTextures["brickCrateTex"]->Resource;

	srvDesc.Shader4ComponentMapping = D3D12_DEFAULT_SHADER_4_COMPONENT_MAPPING;
	srvDesc.Format = brickCrateTex->GetDesc().Format;
	srvDesc.ViewDimension = D3D12_SRV_DIMENSION_TEXTURE2D;
	srvDesc.Texture2D.MostDetailedMip = 0;
	srvDesc.Texture2D.MipLevels = brickCrateTex->GetDesc().MipLevels;
	srvDesc.Texture2D.ResourceMinLODClamp = 0.0f;

	md3dDevice->CreateShaderResourceView(brickCrateTex.Get(), &srvDesc, hDescriptor);
...
void CrateApp::BuildShapeGeometry()
{
	...
	GeometryGenerator::MeshData box = geoGen.CreateBox(1.0f, 1.0f, 1.0f, 40);
...
void CrateApp::BuildRenderItems()
{
	...
	//XMStoreFloat4x4(&boxRitem1->World, XMMatrixTranslation(1.0f, 0.0f, 0.0f));
	//boxRitem1->ObjCBIndex = 1;
	//boxRitem1->Mat = mMaterials["brickCrate"].get();
